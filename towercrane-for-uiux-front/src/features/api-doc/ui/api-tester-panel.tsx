...
const METHOD_STYLE: Record<HttpMethod, CSSProperties> = {
  GET: {
    ...
    backgroundColor:
      "color-mix(in srgb, oklch(0.46 0.15 155) 10%, transparent)",
  ...
  PATCH: {
    ...
    backgroundColor:
      "color-mix(in srgb, oklch(0.52 0.16 300) 10%, transparent)",
    ...
    () => [
      {
        id: "params" as const,
        label: "Params",
        count: countEnabled(content.params),
      },
      {
        id: "headers" as const,
        label: "Headers",
        count: countEnabled(content.headers),
      },
      {
        id: "body" as const,
        label: "Body",
        count: content.body.type === "none" ? 0 : 1,
      },
    ],
            ...
            <div className="fixed inset-0 z-[200] flex flex-col bg-[color:color-mix(in_srgb,var(--background)_40%,transparent)] backdrop-blur-sm">
              <div className="flex min-h-0 flex-1 flex-col overflow-hidden m-4 rounded-lg border border-surface-border-soft bg-surface-raised shadow-2xl">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto p-5">
                  {responseTab === "body" ? (
                    <pre
                      className={`whitespace-pre-wrap break-all font-mono text-sm leading-6 ${response.status === 0 ? "text-destructive" : "text-text-primary"}`}
                    >
                      {isJsonString(response.body)
                        ? prettyJson(response.body)
                        : response.body}
