...
import { Check, GripVertical, Layers2, Loader2, Pencil, Plus, Trash2, X } from 'lucide-react'
import { Card } from '../../../shared/ui/card'
import type { DevChallengeSection } from '../../../entities/dev-challenge/model/types'
import {
  useCreateDevChallengeSection,
  useDeleteDevChallengeSection,
  useDevChallengeSections,
  useReorderDevChallengeSections,
  useUpdateDevChallengeSection,
} from '../lib/hooks'

type ActionState = { type: 'idle' } | { type: 'edit'; value: string } | { type: 'delete' }

type DevChallengeSectionPanelProps = {
  categoryId: string
    ...
    >
      ...
      {isSelected && (
        <div className="ml-1 flex shrink-0 items-center gap-0.5">
          <button onClick={() => onActionChange({ type: 'edit', value: section.title || '' })} className="rounded p-0.5 transition-colors hover:bg-surface-border">
...
export function DevChallengeSectionPanel({
  categoryId,
  selectedSectionId,
  onSelectSection,
}: DevChallengeSectionPanelProps) {
  const [isAdding, setIsAdding] = useState(false)
  const [addValue, setAddValue] = useState('')
  const [actionMap, setActionMap] = useState<Record<string, ActionState>>({})
  const { data: sections = [], isLoading } = useDevChallengeSections(categoryId)
  const createSection = useCreateDevChallengeSection()
  const updateSection = useUpdateDevChallengeSection()
  const deleteSection = useDeleteDevChallengeSection()
  const reorderSections = useReorderDevChallengeSections()
  ...
  const handleAdd = async () => {
    ...
    setAddValue('')
    setIsAdding(false)
    if (section?.id) onSelectSection(section.id)
  }
      ...
      <div className="sticky top-0 border-b border-surface-border bg-surface-muted p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Layers2 className="size-4 text-brand-primary" />
            <p className="text-xs font-bold ui-text-primary">2차 주제 ({sections.length})</p>
          </div>
          <button disabled={!categoryId} onClick={() => setIsAdding(true)} className="rounded p-1 transition-colors hover:bg-surface-border disabled:opacity-40">
