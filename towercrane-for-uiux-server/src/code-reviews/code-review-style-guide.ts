...
- 주요 로직은 #### 단계 제목 → 코드 블록 → 한 줄 설명(레이블 없이 plain text) → → 개선 방향(→ 기호로 시작) 순서로만 작성한다.
...
해당 코드가 맡는 역할을 짧게 설명한다.

→ 다음에 적용할 수 있는 구체적인 개선 방향을 짧게 적는다.
...
커밋 URL, 저장소 URL, 리뷰할 로직 설명을 서버 분석 요청으로 넘기는 진입점이다.

→ 요청 payload가 늘어나면 폼 상태와 분석 요청 payload 생성 로직을 분리한다.
...
커밋 diff 수집, 변경 파일 파싱, 리뷰 생성, 저장을 한 흐름으로 연결한다.

→ diff 파싱과 리뷰 저장 책임이 커지면 별도 서비스로 분리한다.
